...
*                 to OSIntEnter() at the beginning of the ISR you MUST have a call to () at the
*                 end of the ISR.
...
void  OSStart (void)
{
    if (OSRunning == OS_FALSE) {
        OS_SchedNew();
		//OS_SchedEDF();       /* Find highest priority's task priority number   */
...
/*--------------------------------------------------------------------------
此函数一般是在调度任务时调用的（代替OS_SchedNew函数）
  找到deadline最早的任务
  假设当这个函数被调用的时候中断时被禁止的
  这个函数的作用是更新当前系统中最高优先级的序号OSPrioHighRdy
  --------------------------------------------------------------------------*/
static void OS_SchedEDF(void) {
	OS_TCB* p_current;
	OS_TCB* edf_ptcb;
...
/*
此函数用于获取下一个EDF任务的id（需要根据截止时间找到截止时间最近的那个任务，并将当前任务设为这个任务）
*/
static INT32U getEDFNextID(void) {
	OS_TCB* p_current;
	OS_TCB* edf_ptcb;
